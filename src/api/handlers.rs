...
pub async fn get_balance(
    ...
    let balance = pi_network::BalanceInfo {
        ...
        locked: vec![
            pi_network::LockedBalance {
                amount: 10000000, // 10 PI
                unlock_time: Utc::now() + chrono::Duration::hours(1),
...
pub async fn withdraw(
    ...
    // Mock wallet seed (in real implementation, extract from session)
    let wallet_seed = "mock_wallet_seed".to_string();
...
pub async fn claim(
    ...
    let claiming_engine = state.claiming_engine.clone();
    let log_sender = state.log_sender.clone();
    let wallet_seed = "mock_wallet_seed".to_string(); // Extract from session in real implementation
...
pub async fn get_transactions() -> Result<Json<Vec<TransactionInfo>>, StatusCode> {
    // Mock transactions
    let transactions = vec![
        TransactionInfo {
            ...
            to_address: Some("GABC...XYZ".to_string()),
        },
        TransactionInfo {
            id: "tx_2".to_string(),
